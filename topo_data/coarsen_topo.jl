...
d = NCDataset(joinpath(@__DIR__, "ETOPO_2022_v1_60s_N90W180_surface.nc"))
lon = d["lon"][:]
lat = d["lat"][:]
z = Float64.(d["z"][:, :])
close(d)

# d = NCDataset(joinpath(@__DIR__, "ETOPO_2022_v1_60s_N90W180_geoid.nc"))
# lon_geoid = d["lon"][:]
# lat_geoid = d["lat"][:]
# z_geoid = Float64.(d["z"][:, :])
# close(d)
...
function plot_H(lon, lat, z, fname; vmax=6000)
    ...
    heatmap(lon, lat, H', dpi=300,
            ...
            clims=(0, vmax), c=cgrad(:magma, rev=true),
...
@info string("(nlon, nlat) = ", size(z_global))
plot_H(lon_global, lat_global, z_global, joinpath(@__DIR__, "H.png"))
ofile = joinpath(@__DIR__, "etopo_global_$(arcmins)min.jld2")
jldsave(ofile; lon=lon_global, lat=lat_global, z=z_global)
@info "Saved '$ofile'"

# # coarsen and zoom in to Black Sea region 
# arcmins = 5 # 5 arc-minutes (1/12th degree)
# lon_box, lat_box, z_box = coarsen_topo(lon, lat, z; arcmins)
# @info string("(nlon, nlat) = ", size(z_box))
...
# plot_H(lon_box, lat_box, z_box, joinpath(@__DIR__, "H_blacksea.png"); vmax=2000)
# ofile = joinpath(@__DIR__, "etopo_blacksea_$(arcmins)min.jld2")
# jldsave(ofile; lon=lon_box, lat=lat_box, z=z_box)
# @info "Saved '$ofile'"