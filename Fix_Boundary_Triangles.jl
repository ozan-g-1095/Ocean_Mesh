...
# function order_the_surface_lines(unordered_lines, ordered_lines)
#     while !isempty(unordered_lines)
#         initial = unordered_lines[1][1]
#         current = unordered_lines[1][2]
#         index = 1
#         loop_no = 1
#         push!(ordered_lines[loop_no], unordered_lines[index])
#         deleteat!(unordered_lines, index)
#         while initial != current 
#             for (idx, elem) in enumerate(unordered_lines)
#                 if elem[1] == current 
#                     current = elem[2]
#                     index = idx
#                     break
...
function fix_boundary(curve_to_tri, bound_nodes, added_points)
    for (loop_index, loop) in enumerate(edges_on_the_boundary_ordered)
        ...
        # for i in (1:length(loop))
            ...
            # println("node1 ", node1_ID, " node2 ", node2_ID ," node3 ", third_node)
            #

            # println(" try_reaching_the_node: ", try_reaching_the_node )
...
# for index in bndix
#     edge = unique_edges[index,:]
#     # println("edge: ", edge)
#     push!(boundary_edges, (edge[1], edge[2], false))
# end
gmsh.initialize()
gmsh.model.add("Boundary_Fixed.msh")
...
# for point in points
#     gmsh.model.occ.addPoint(point[1], point[2], point[3], point[4], point[5])
# end

# create_loop(boundary_edges, ordered_boundary_edges)
...
function fix_the_boundary()
    for loops in ordered_surface_lines
        for i in (1:length(loops))
        current_node_1 = loops[i][2]
        current_node_2 = loops[i][3]
            for triangle in triangles
                if current_node_1 in triangle && current_node_2 in triangle && triangle[1] in nodes_on_the_boundary && triangle[2] in nodes_on_the_boundary && triangle[3] in nodes_on_the_boundary
...
@save "boundary_edges_ordered" boundary_edges_ordered

gmsh.model.occ.synchronize()
gmsh.model.mesh.generate(3)
gmsh.write("Boundary_Fixed.msh")
gmsh.finalize()